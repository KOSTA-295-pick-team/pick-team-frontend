...
export const VideoConferencePage: React.FC = () => {
  const { currentUser } = useAuth(); // allUsersForChat 제거
    ...
    if (currentUser && workspaceId) {
        const fetchParticipants = async () => {
            try {
                ...
                // 현재 유저를 가장 앞에, 나머지는 members 리스트에서
                const otherParticipants = members.filter(m => m.id !== currentUser.id);
                setParticipants([currentUser, ...otherParticipants]);
            } catch (error) {
                console.error("Failed to fetch participants:", error);
                // 에러 발생 시 현재 유저만이라도 표시
